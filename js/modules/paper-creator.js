(function () {
  ...
  function createRequestToken(prefix = 'request') {
    const safePrefix = String(prefix || 'request').replace(/[^a-z0-9_-]+/gi, '-').slice(0, 24) || 'request';
    if (window.crypto?.randomUUID) return `${safePrefix}-${window.crypto.randomUUID()}`;
    const random = window.crypto?.getRandomValues ? window.crypto.getRandomValues(new Uint32Array(4)) : null;
    const entropy = random ? Array.from(random).map((value) => value.toString(16).padStart(8, '0')).join('') : `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    return `${safePrefix}-${entropy}`;
  }

  function sanitizeClipboardHtml(rawHtml) {
    const source = String(rawHtml || '');
    if (!source.trim()) return { html: '', removedImages: 0, simplified: false };
    if (new Blob([source]).size > 2 * 1024 * 1024) {
      throw new Error('The copied content is too large to paste at once. Paste it section by section.');
    }

    const template = document.createElement('template');
    template.innerHTML = source;
    template.content.querySelectorAll('script,style,link,meta,base,iframe,object,embed,form,input,button,textarea,select,option,noscript').forEach((node) => node.remove());

    const allowedTags = new Set(['P', 'DIV', 'BR', 'STRONG', 'B', 'EM', 'I', 'U', 'SUP', 'SUB', 'OL', 'UL', 'LI', 'TABLE', 'THEAD', 'TBODY', 'TFOOT', 'TR', 'TH', 'TD', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'SPAN', 'BLOCKQUOTE']);
    const blockTags = new Set(['P', 'DIV', 'OL', 'UL', 'LI', 'TABLE', 'TR', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'BLOCKQUOTE']);
    let removedImages = 0;
    ...
    function safeStyle(element) {
      ...
      if (['left', 'right', 'center', 'justify'].includes(align)) declarations.push(`text-align:${align}`);
      const weight = String(style.fontWeight || '').toLowerCase();
      if (weight === 'bold' || Number.parseInt(weight, 10) >= 600) declarations.push('font-weight:700');
      if (String(style.fontStyle || '').toLowerCase() === 'italic') declarations.push('font-style:italic');
      if (String(style.textDecorationLine || style.textDecoration || '').toLowerCase().includes('underline')) declarations.push('text-decoration:underline');
      const vertical = String(style.verticalAlign || '').toLowerCase();
      if (vertical === 'super' || vertical === 'sub') declarations.push(`vertical-align:${vertical}`);
    ...
    function cleanNode(node) {
      ...
      if (tag === 'IMG') {
        ...
        if (alt) replacement.textContent = `[Image: ${alt}]`;
        return replacement;
      }

      const children = document.createDocumentFragment();
      Array.from(node.childNodes).forEach((child) => children.appendChild(cleanNode(child)));
      ...
      if ((tag === 'TD' || tag === 'TH') && /^\d{1,2}$/.test(String(node.getAttribute('colspan') || ''))) clean.setAttribute('colspan', node.getAttribute('colspan'));
      if ((tag === 'TD' || tag === 'TH') && /^\d{1,2}$/.test(String(node.getAttribute('rowspan') || ''))) clean.setAttribute('rowspan', node.getAttribute('rowspan'));
    ...
    output.querySelectorAll('span:empty,p:empty,div:empty').forEach((node) => {
      if (!node.querySelector('br')) node.remove();
    });
    output.querySelectorAll('div').forEach((node) => {
      if (!node.textContent.trim() && !node.querySelector('br,table,ol,ul')) node.remove();
    });
    output.querySelectorAll('*').forEach((node) => {
      if (!blockTags.has(node.tagName) && node.tagName !== 'BR' && !node.textContent && !node.children.length) node.remove();
    });
    return { html: output.innerHTML.trim(), removedImages, simplified };
  }
  ...
  async function handlePaste(event) {
    ...
    const plainText = String(clipboard?.getData('text/plain') || '');
    const richHtml = String(clipboard?.getData('text/html') || '');
    const hasTextContent = Boolean(plainText.trim() || richHtml.trim());

    if (!hasTextContent && !files.length) {
      if (imageClipboard) {
        event.preventDefault();
        pasteCopiedImage();
      }
    ...
    try {
      if (richHtml.trim()) {
        const cleaned = sanitizeClipboardHtml(richHtml);
        removedImages = cleaned.removedImages;
        simplified = cleaned.simplified;
        if (cleaned.html && normalize(cleaned.html.replace(/<[^>]+>/g, ' '))) insertHtml(cleaned.html);
        else if (plainText.trim()) insertText(plainText);
      } else if (plainText) {
        insertText(plainText.replace(/\r\n?/g, '\n'));
      }

      for (const file of files.slice(0, 5)) await insertImageFile(file);
      if (files.length > 5) toast('Only the first 5 pasted images were inserted.', 'error');
      if (removedImages && !files.length) toast('Text pasted safely. Linked images were removed; use Insert Image to add them.', 'error');
      else if (simplified) toast('Content pasted with safe formatting. Unsupported Word/website formatting was removed.');
    } catch (error) {
      if (plainText.trim()) {
        insertText(plainText.replace(/\r\n?/g, '\n'));
        toast('Rich formatting could not be used, so the questions were pasted as plain text.', 'error');
      } else {
        toast(error.message || 'The copied content could not be pasted.', 'error');
      }
    }
  }
  ...
  function openUploadModal(options) {
    ...
    // A Principal-requested correction is an existing workflow item, so the
    // teacher must be able to resubmit it even when new uploads are closed.
    if (!correctionPaper && settings?.permissions?.canUpload === false) {
  ...
  async function submitUpload(event) {
    ...
    try {
      ...
      if (!uploadRequestId) uploadRequestId = createRequestToken(extension === 'docx' ? 'docx-import' : 'paper-upload');
      const payload = { className, subject, exam, chapters, maxMarks, timeAllowed, note: normalize(byId('uploadPaperNote')?.value), fileName: file.name, mimeType: file.type, fileSize: file.size, fileBase64, requestId: uploadRequestId, ...(uploadRevisionContext || {}) };
      if (extension === 'docx') {
